...
namespace PIE.Meteo.RasterProject
{
    public class RasterProjector : IRasterProjector
    {
        ...
        public void Transform(SpatialReference srcSpatialRef, double[] srcXs, double[] srcYs,
            SpatialReference dstSpatialRef)
        {
            using (IProjectionTransform transform =
                ProjectionTransformFactory.GetProjectionTransform(srcSpatialRef, dstSpatialRef))
            {
                if (srcSpatialRef.IsSame(dstSpatialRef) != 1)
                    transform.Transform(srcXs, srcYs);
            }
        }

        public void ComputeDstEnvelope(SpatialReference srcSpatialRef, double[] srcXs, double[] srcYs, Size srcSize,
        ...
        public void ComputeDstEnvelope(SpatialReference srcSpatialRef, double[] srcXs, double[] srcYs, Size srcSize,
            SpatialReference dstSpatialRef, out PrjEnvelope dstEnvelope, Action<int, string> progressCallback)
        {
            ComputeDstEnvelope(srcSpatialRef, srcXs, srcYs, srcSize, dstSpatialRef, null, out dstEnvelope,
                progressCallback);
        }

        public static PrjEnvelope GetEnvelope(double[] srcXs, double[] srcYs, SpatialReference dstSpatialRef,
        ...
        */
        ...
        /// </summary>
        public void ComputeIndexMapTable(SpatialReference srcSpatialRef, double[] srcXs, double[] srcYs,Size srcSize, Size srcDataSize,
                                        SpatialReference dstSpatialRef, Size dstSize, PrjEnvelope dstEnvelope,
                                        out UInt16[] rowMapTable, out UInt16[] colMapTable, Action<int, string> progressCallback)
        {
            PrjEnvelope maxDstEnvelope;
        ...
        */
                        ...
                        for (int oldCol = 0; oldCol < srcWidth; oldCol++)
                        {
                                ...
                                srcX <= srcValidMaxX)
                            {
                                ...
                                for (int blockRow = 0; blockRow < slopeY; blockRow++)
                                {
                                    ...
                                    i = (int) oldRowF;
                                    if (i > oldRowF) --i; //x = (long)Math.Floor(fx);//取最大整数
                                    u = oldRowF - i;
                                    pu1 = 1 - u;
        ...
        /// <param name="progressCallback"></param>
        public void Project<T>(T[] srcRaster, Size srcSize, UInt16[] rowMapTable, UInt16[] colMapTable, Size mapSize,
            T[] dstRaster, T destFillValue, UInt16 mapTableInvalidValue, Action<int, string> progressCallback)
        {
        ...
        public void Project<T>(T[] srcRaster, Size srcSize, UInt16[] rowMapTable, UInt16[] colMapTable, Size mapSize,
            T[] dstRaster, T destFillValue, Action<int, string> progressCallback)
        {
            int lutHeight = mapSize.Height;
            int lutWidth = mapSize.Width;
            int srcHeight = srcSize.Height;
        ...
        public void ProjectNew<T>(T[,] srcRaster, Size srcSize, UInt16[] rowMapTable, UInt16[] colMapTable,
            Size mapSize, T[,] dstRaster, T destFillValue)
        {
            int lutHeight = mapSize.Height;
            int lutWidth = mapSize.Width;
            int srcHeight = srcSize.Height;
        ...
        /// <returns></returns>
        public bool HasVaildEnvelope(double[] xs, double[] ys, PrjEnvelope validEnv, SpatialReference oSpatialRef,
            SpatialReference tSpatialRef)
        {
