...
namespace PIE.Meteo.RasterProject
{
    ...
    public class PrjEnvelope : ICloneable
    {
        ...
        public PrjEnvelope()
        {
            ...
            SpatialReference _srs = new SpatialReference("");
            _srs.ImportFromProj4("+proj=latlong +datum=WGS84 +no_defs");
        }

        public PrjEnvelope(double minX, double maxX, double minY, double maxY, SpatialReference srs)
        {
            _minX = minX;
            _maxX = maxX;
            _minY = minY;
        ...
        public PrjEnvelope(float minX, float maxX, float minY, float maxY, SpatialReference srs)
        {
            _minX = minX;
            _maxX = maxX;
            _minY = minY;
        ...
        public static PrjEnvelope CreateByCenter(SpatialReference srs, double centerX, double centerY, double width, double height)
        {
            ...
            return new PrjEnvelope(minX, maxX, minY, maxY, srs);
        }

        public static PrjEnvelope CreateByLeftBottom(SpatialReference srs, double leftBottomX, double leftBottomY, double width, double height)
        {
        ...
        public static PrjEnvelope CreateByLeftTop(SpatialReference srs, double leftTopX, double leftTopY, double width, double height)
        {
        ...
        public static PrjEnvelope Empty
        {
            get
            {
                ...
                return new PrjEnvelope(0.0d, 0.0d, 0.0d, 0.0d, srs);
            }
        }
        ...
        {
            return new PrjEnvelope(_minX, _maxX, _minY, _maxY, _srs);
        }
        ...
        public static PrjEnvelope Intersect(PrjEnvelope a, PrjEnvelope b)
        {
            ...
            if (miny > maxy)
                return null;
            return new PrjEnvelope(minx, maxx, miny, maxy, a._srs);
        }
        ...
        public static PrjEnvelope GetEnvelope(double[] srcXs, double[] srcYs, PrjEnvelope validEnvelope, double xFringe, double yFringe)
        {
            ...
            PrjEnvelope dstEnvelope = new PrjEnvelope(curMinX, curMaxX, curMinY, curMaxY, validEnvelope._srs);
            return dstEnvelope;
        }
        ...
        public static bool HasValidEnvelope(double[] srcXs, double[] srcYs, PrjEnvelope validEnv, out double validRate, out PrjEnvelope outEnv)
        {
            ...
            outEnv = new PrjEnvelope(curMinX, curMaxX, curMinY, curMaxY, validEnv._srs);
            return true;
        }
        ...
        public static bool Transform(PrjEnvelope env, SpatialReference spatialRef, out PrjEnvelope newEnv)
        {
            SpatialReference oldSrs = null;
            if (env.Srs != null)
            {
                oldSrs = env.Srs;
            }
            ...
            {
                oldSrs = new SpatialReference("");
                oldSrs.ImportFromEPSG(4326);

            }

            try
            {
                RasterProject.ProjectionTransform trans = new ProjectionTransform(oldSrs, spatialRef);
                double[] x = new double[] { env.MaxX, env.MinX };
                double[] y = new double[] { env.MaxY, env.MinY };
                trans.Transform(x, y);
                newEnv = new PrjEnvelope(x[1], x[0], y[1], y[0], spatialRef);
                return true;
