...
namespace PIE.Meteo.RasterProject
{
    ...
    public class Proj4Projection : IDisposable
    {
        ...
        public static void MultiTransform(Proj4Projection src, Proj4Projection dst,
        double[] x, double[] y, double[] z)
        {
            lock (lockObj)
            {
                ...
                int PART_SIZE = 500000;
                int nParts = x.Length / PART_SIZE + 1;
                int result = 0;
                //List<double[]> partXList = new List<double[]>();
                //List<double[]> partYList = new List<double[]>();
                //for (int i = 0; i < nParts; i++)
                //{
                //    int start = i * PART_SIZE;
                //    int length = i == nParts - 1 ? x.Length - start : PART_SIZE;
                //    double[] partX = new double[length];
                //    double[] partY = new double[length];
                ...
                //    partYList.Add(partY);
                //}
                string srcProjString = src.ToString();
                string dstProjString = dst.ToString();

                Parallel.For(0, nParts, (i) =>
                {
                    int start = i * PART_SIZE;
                    int length = i == nParts - 1 ? x.Length - start : PART_SIZE;
                    double[] partX = new double[length];
                    double[] partY = new double[length];
                    Array.Copy(x, start, partX, 0, length);
                    Array.Copy(y, start, partY, 0, length);
                    IntPtr ctx = Proj.pj_ctx_alloc();
                    IntPtr src_ctx = Proj.pj_init_plus_ctx(ctx, srcProjString);
                    IntPtr dst_ctx = Proj.pj_init_plus_ctx(ctx, dstProjString);
                    result += Proj.pj_transform(src.prj, dst.prj, partX.Length, 1, partX, partY, null);
                    Proj.pj_free(src_ctx);
                    Proj.pj_free(dst_ctx);
                    Proj.pj_ctx_free(ctx);
                    Array.Copy(partX, 0, x, start, length);
                    Array.Copy(partY, 0, y, start, length);
                });
                //for (int i = 0; i < nParts; i++)
                //{
                //    int start = i * PART_SIZE;
                ...
                //    for (int j = 0; j < length; j++)
                //    {
                //        x[start + j] = partXList[i][j];
                //        y[start + j] = partYList[i][j];
                //    }
                //}

                //partXList = new List<double[]>();
                //partYList = new List<double[]>();
