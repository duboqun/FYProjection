...
namespace HDF.PInvoke
{
    ...
    internal class H5UnixDllImporter : H5DLLImporter
    {
        ...
        public H5UnixDllImporter(string libName)
        {
            ...
            var ld_path = Environment.GetEnvironmentVariable("LIBRARY_PATH") ?? string.Empty;
            if (!ld_path.Contains("/usr/local/lib"))
            {
                Environment.SetEnvironmentVariable("LIBRARY_PATH", "/usr/local/lib:" + ld_path);
            }
            ...
            {
                IntPtr er = dlerror();
                string erMsg = Marshal.PtrToStringAnsi(er);
