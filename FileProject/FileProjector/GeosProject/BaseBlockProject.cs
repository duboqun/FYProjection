...
{
    public abstract class BaseBlockProject : FileProjector
    {
        ...
        protected IBlockProcess _process;

        protected AbstractWarpDataset _prdWriter { get; set; }

        protected AbstractWarpDataset _srcRaster { get; set; }
        private List<AbstractWarpDataset> _srcDatasetList = new List<AbstractWarpDataset>();
        public BaseBlockProject()
        {
            _name = "BaseBlockProject";
            _fullname = "条带静止卫星投影";
        ...
        private void ProjectRaster(AbstractWarpDataset srcRaster, FilePrjSettings prjSettings, AbstractWarpDataset prdWriter, Action<int, string> progressCallback, UInt16 fillValue, int beginBandIndex, double dataWeight, float zoom)
        {
            ...
            for (int oRow = 0; oRow < outHeight; oRow += rowStep)
            {
                ...
                MyLog.Log.Print.Info("开始创建像元经纬度与查找表数组");
                //经纬度数据
                double[] xs, ys;
                UInt16[] rows, cols;
                try
                ...
                MyLog.Log.Print.Info("结束创建像元经纬度与查找表数组");
                ...
                #region 初始化查找表
                {
                    ...
                    MyLog.Log.Print.Info($"坐标投影转换：{timer.ElapsedMilliseconds}ms");
        ...
        {
            foreach (var item in _srcDatasetList)
            {
                item?.Dispose();
            }
            if (_prdWriter != null)
        ...
        protected IRasterBand[] TryCreateRasterDataBands(AbstractWarpDataset srcRaster, FilePrjSettings prjSettings, Action<int, string> progressCallback)
        {
            ...
            for (int i = 0; i < prjSettings.OutBandNos.Length; i++)
            {
                ...
                AbstractWarpDataset ds = _process.GetDataset(bandNo);
                _srcDatasetList.Add(ds);
        ...
        private void ReadBandData<T>(T[] bandData, int bandIndex, int xOffset, int yOffset, int blockWidth, int blockHeight, PixelDataType dataType)
        {
            ...
            {
                //对前4参数进行修改
                int srcWidth = _srcRaster.Width;
                int srcHeight = _srcRaster.Height;
                int curWidth = latBand.GetXSize();
                int curHeight = latBand.GetYSize();
                double xZoomFactor = srcWidth * 1.0 / curWidth;
                double yZoomFactor = srcHeight * 1.0 / curHeight;

                //原始数据是1000的，现在到了500分辨率   xZ = 0.5 yZ =0.5

                bool readState = latBand.Read(
                    Convert.ToInt32(xOffset / xZoomFactor),
                    Convert.ToInt32(yOffset / yZoomFactor),
                    Convert.ToInt32(blockWidth / xZoomFactor),
                    Convert.ToInt32(blockHeight / yZoomFactor),
                    bandData, blockWidth, blockHeight, dataType);
                if (!readState)
                {
                    System.Diagnostics.Debug.WriteLine("文件读取失败");
                    MyLog.Log.Print.Error("文件读取失败:BaseBlockProject ReadBandData");
                }
            }
            finally
            {
