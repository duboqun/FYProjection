...
namespace PIE.Meteo.FileProject
{
    public class FY1X_1A5FileProjector : FileProjector
    {
        ...
        private void ReadySensorZenith(WarpDataset srcRaster)
        {
        ...
        private void ProjectToLDF(WarpDataset srcRaster, NOAA_PrjSettings prjSettings, SpatialReference dstSpatialRef, Action<int, string> progressCallback)
        {
            ...
            using (IRasterDataDriver drv = GeoDataDriver.GetDriverByName("LDF") as IRasterDataDriver)
            {
                ...
                using (WarpDataset prdWriter = drv.Create(outfilename, dstSize.Width, dstSize.Height, dstBandCount, enumDataType.UInt16, opts.ToArray()) as WarpDataset)
                {
                    float outResolutionX = prjSettings.OutResolutionX;
                    float outResolutionY = prjSettings.OutResolutionY;
                                ...
                                IRasterBand band = prdWriter.GetRasterBand(i + 1);
                                {
                                    unsafe
                                    {
                                        fixed (UInt16* ptr = dstBandData)
                                        {
                                            ...
                                            band.Write(blockOffsetX, blockOffsetY, blockWidth, blockHeight, bufferPtr,  blockWidth, blockHeight, PixelDataType.UInt16);
                                        }
                                    }
                                }
                            }
        ...
        private void ReadBandData(UInt16[] bandData, WarpDataset srcRaster, int bandNumber, int xOffset, int yOffset, int blockWidth, int blockHeight)
        {
        ...
        //读取定位信息(经纬度数据集)
        protected override void ReadLocations(WarpDataset srcRaster, out double[] longitudes, out double[] latitudes, out Size locationSize)
        {
            longitudes = null;
            latitudes = null;
            WarpDataset _longitudeRaster = null;
            WarpDataset _latitudeRaster = null;
            try
            {
                ...
                {
                    var ds =DataSource.DatasetFactory.OpenDataset(longCacheFilename, OpenMode.ReadOnly);
                    _longitudeRaster = new WarpDataset(ds);
                ...
                {
                    var ds = DataSource.DatasetFactory.OpenDataset(latCacheFilename, OpenMode.ReadOnly);
                    _latitudeRaster = new WarpDataset(ds);
        ...
        {
            IRasterBand band = raster.GetRasterBand(bandNo);
            {
                ...
                unsafe
                {
                    ...
                    {
                        IntPtr bufferPtr = new IntPtr(ptr);
                        band.Read(0, 0, band.GetXSize(), band.GetYSize(), bufferPtr,band.GetXSize(), band.GetYSize(), PixelDataType.Float64);
                    }
                }
                return data;
            }
        ...
        private IRasterBand WriteCacheFile(double[] data, string fileName, int width, int height, out WarpDataset cacheWriter)
        {
            ...
            cacheWriter = _outLdfDriver.Create(fileName, width, height, 1, enumDataType.Double, options) as WarpDataset;
        ...
        private Double[] ReadBandByName(WarpDataset srcRaster, string bandName)
        {
        ...
        private Int16[] ReadBandByNameToInt16(WarpDataset srcRaster, string bandName)
        {
            IBandProvider bandPrd = srcRaster.BandProvider;
            IRasterBand[] bands = bandPrd.GetBands(bandName);
            if (bands == null || bands.Length == 0 || bands[0] == null)
            ...
            try
            {
                ...
                {
                    Int16[] data = new Int16[band.GetXSize() * band.GetYSize()];
                    unsafe
                    {
                        fixed (Int16* ptr = data)
                        {
                            IntPtr bufferPtr = new IntPtr(ptr);
                            band.Read(0, 0, band.GetXSize(), band.GetYSize(), bufferPtr, band.GetXSize(), band.GetYSize(), PixelDataType.Int16);
                        }
                    }
                    return data;
                }
        ...
        #region 定标参数读取
        //准备[辐射定标]参数
        private void ReadyRadiationArgs(WarpDataset srcRaster)
        {
        }

        private void ReadySolarZenithArgsToFile(WarpDataset srcRaster)
        {
            //if (srcRaster == null)
        ...
        private void WriteData(float[] data, string fileName, int width, int height)
        {
            ...
            if (_outLdfDriver == null)
                _outLdfDriver = GeoDataDriver.GetDriverByName("LDF") as IRasterDataDriver;
            WarpDataset cacheWriter = _outLdfDriver.Create(fileName, width, height, 1, enumDataType.Float, options) as WarpDataset;
            {
                IRasterBand band = cacheWriter.GetRasterBand(1);
                {
                    unsafe
        ...
        private float[] ReadFileAttributeToFloat(WarpDataset srcbandpro, string AttrName, int length)
        {
        ...
        private Int16[] ReadDataSetToInt16(WarpDataset srcbandpro, Size srcSize, string dataSetName, int bandIndex)
        {
        ...
        private float[] ReadDataSetToSingle(WarpDataset srcbandpro, Size srcSize, string dataSetName, int bandIndex)
        {
        ...
        public override WarpDataset Project(WarpDataset srcRaster, FilePrjSettings prjSettings, WarpDataset dstRaster, int beginBandIndex, Action<int, string> progressCallback)
        {
        ...
        protected override void DoRadiation(WarpDataset srcImgRaster, int i, ushort[] srcBandData, float[] solarZenithData, Size srcBlockImgSize, Size angleSize)
        {
            throw new NotImplementedException();
        }
    }
