...
namespace PIE.Meteo.FileProject
{
    internal class NOAAFileProjector : FileProjector
    {
        ...
        private void DoSession(WarpDataset srcRaster, ISpatialReference dstSpatialRef,FY3_VIRR_PrjSettings fy3prjSettings, Action<int, string> progressCallback)
        {
        ...
        private void Project(WarpDataset srcRaster, FY3_VIRR_PrjSettings prjSettings, ISpatialReference dstSpatialRef, Action<int, string> progressCallback)
        {
        ...
        /// 2、对所有波段执行投影
        /// </summary>
        private void ProjectToLDF(WarpDataset srcRaster, FY3_VIRR_PrjSettings prjSettings, ISpatialReference dstSpatialRef, Action<int, string> progressCallback)
        {
            ...
            using (IRasterDataDriver drv = GeoDataDriver.GetDriverByName("LDF") as IRasterDataDriver)
            {
                ...
                using (WarpDataset prdWriter = drv.Create(outfilename, dstSize.Width, dstSize.Height, dstBandCount,
                    enumDataType.UInt16, "INTERLEAVE=BSQ", "VERSION=LDF", "SPATIALREF=" + dstProj4, mapInfo, "WITHHDR=TRUE") as WarpDataset)
                {
                    //计算查找表
        ...
        //读取定位信息(经纬度数据集)
        private void ReadLongitudeLatitude(WarpDataset srcRaster, out double[] longitudes, out double[] latitudes)
        {
            IBandProvider srcbandpro = srcRaster.BandProvider as IBandProvider;
            {
        ...
        private void ReadySolarZenithArgs(WarpDataset srcRaster)
        {
            if (srcRaster == null)
        ...
        public override WarpDataset Project(WarpDataset srcRaster, FilePrjSettings prjSettings, WarpDataset dstRaster, Action<int, string> progressCallback)
        {
        ...
        private void TryCreateDefaultArgs(WarpDataset srcRaster, FY3_VIRR_PrjSettings prjSettings, ref ISpatialReference dstSpatialRef)
        {
