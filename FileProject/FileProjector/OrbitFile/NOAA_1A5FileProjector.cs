    ...
    public class NOAA_1A5FileProjector : FileProjector
    {
        ...
        private void ProjectToLDF(WarpDataset srcRaster, NOAA_PrjSettings prjSettings, ISpatialReference dstSpatialRef, Action<int, string> progressCallback)
        {
            ...
            using(IRasterDataDriver drv = GeoDataDriver.GetDriverByName("LDF") as IRasterDataDriver)
            {
                ...
                using (WarpDataset prdWriter = drv.Create(outfilename, dstSize.Width, dstSize.Height, dstBandCount, enumDataType.UInt16, options) as WarpDataset)
                {
        ...
        protected override void ReadLocations(WarpDataset srcRaster, out double[] longitudes, out double[] latitudes,out Size locationSize)
        {
            ...
            try
            {
                ...
                {
                    _longitudeRaster = GeoDataDriver.Open(longCacheFilename) as WarpDataset;
